...
<pre> ```text cpp-sentiment-engine/ ├── src/ │ ├── main.cpp │ ├── preprocess.cpp │ └── features.cpp ├── include/ │ ├── preprocess.h │ └── features.h ├── data/ │ ├── sample.json │ ├── dataset.json │ └── sentiment_lexicon.json ├── external/ │ └── json.hpp └── CMakeLists.txt ``` </pre>
